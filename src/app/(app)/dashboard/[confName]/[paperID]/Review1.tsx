...
export function Review1Dialog({ Review1, paperID }: { Review1: string; paperID: string }) {
  ...
  const addEmail = async () => {
    ...
    try {
      ...
      const response = await axios.get(`/api/check-valid-user-and-reviewer?userEmail=${emailInput}`);
      ...
      toast({
        title: "Error",
        description:
          error.response?.data?.message || "An unexpected error occurred while validating the email.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const response = await axios.patch(`/api/add-reviewer-id-and-send-mail?paperID=${paperID}`, {
        reviewerIds: userIDlist,
      });
  
      toast({
        title: "Success",
        description: response.data.message || "Reviewers have been assigned successfully.",
        variant: "default",
      });
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Assign reviewers and make changes to your review here. Click save when you&apos;re done.
          </DialogDescription>
        </DialogHeader>
