...
const EditConferencePopup = ({
  conferenceDetails,
}: EditConferencePopupProps) => {
  ...
  const onSubmit = async (data: z.infer<typeof conferenceSchema>) => {
    ...
    console.log(data);
    try {
      const response = await updateConference({
        confAcronym: params.confAcronym,
        conferenceDetails: data,
      }).unwrap(); // Use .unwrap() to directly get the fulfilled response or throw an error if it failed
      toast({
        title: 'Success',
