...
export const ConferenceDashboardPaperSlice = createApi({
    ...
    tagTypes: ["addcomment"],
    endpoints: (builder) => ({
      getPaperDetailsByPaperID: builder.query<PaperDetails, string>({ // pahla parameter kya response aayega vo batata hai, second parameter kya input dere ho in query vo batata hai
      ...
      sendCommentForPaper: builder.mutation<ApiResponse<null>, InputParamsTypeForSendComment>({
        query: (InputParams) => ({
          url: '/add-comment',
          method: 'PATCH',
          body: {
