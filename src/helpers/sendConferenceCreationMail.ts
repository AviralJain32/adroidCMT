...
export async function sendConferenceCreationMail(
  ...
  try {
    await sgMail.send({
      ...
      html: render(
        ConferenceCreationEmailTemplate({
          username: conferenceOrganizer,
          conferenceTitle,
          conferenceDate: conferenceStartDate,
        }),
      ),
    });
    ...
    return {
      success: true,
      message: 'conference creation email send sucessfully',
    };
  } catch (emailError) {
    console.log('error sending verification email', emailError);
    return {
      success: false,
      message: 'failed to send confernece creation email',
    };
  }
}
